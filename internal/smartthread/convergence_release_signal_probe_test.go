...
// complete path (leave active → RemoveTask → next tick) keeps prevActiveGids so
// bandwidthRelease and windowInvalidated can observe the disappearance.
//
...
func TestConvergence_RemoveTask_PreservesDisappearanceSignal(t *testing.T) {
	...
	// Production complete path: leave active set, then RemoveTask (handleTaskComplete).
