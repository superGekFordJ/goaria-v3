...
func TestAuthProfileStoreSetSnapshotAndResolveRedacted(t *testing.T) {
	...
	if !snapshot.HasSecret || snapshot.Kind != AuthSecretKindBearer || snapshot.PackID != "fixturepack" || snapshot.ProfileID != "default" {
		t.Fatalf("snapshot = %#v", snapshot)
	}

	snapshots, err := store.AuthProfileSnapshots(context.Background(), "fixturepack")
	if err != nil {
	...
	resolved, err := store.ResolveAuthProfile(context.Background(), "fixturepack", "default", "https://api.fixture.invalid/path")
	if err != nil {
...
func TestAuthProfileStorePersistsAndReloads(t *testing.T) {
	...
	resolved, err := reloaded.ResolveAuthProfile(context.Background(), "fixturepack", "default", "https://fixture.invalid/d/abc")
	if err != nil {
		t.Fatalf("ResolveAuthProfile() error = %v", err)
	}
...
func TestAuthProfileStoreRejectsInvalidInputs(t *testing.T) {
	...
	if _, err := store.ResolveAuthProfile(context.Background(), "fixturepack", "default", "https://evil.test/path"); err == nil {
...
func TestAuthProfileStoreClearRemovesToken(t *testing.T) {
	...
	if err := store.ClearAuthProfile(context.Background(), "fixturepack", "default"); err != nil {
...
func TestAuthProfileStoreErrorsAndSnapshotsDoNotExposeSecrets(t *testing.T) {
	...
	_, err = store.ResolveAuthProfile(context.Background(), "fixturepack", "other", "https://evil.test/?token="+secret)
	if err == nil {
...
func TestAuthProfileStoreConcurrentOperations(t *testing.T) {
	...
	for i := range 16 {
		...
		go func(i int) {
			...
			_, _ = store.AuthProfileSnapshots(ctx, "fixturepack")
			_, _ = store.ResolveAuthProfile(ctx, "fixturepack", profileID, "https://api.fixture.invalid/path")
			if i%3 == 0 {
				_ = store.ClearAuthProfile(ctx, "fixturepack", profileID)
			}
		}(i)
	}
