...
func TestIdempotency_HitSamePayload(t *testing.T) {
	...
	st, _, _ := c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", digest)
	if st != idempMiss {
	...
	st, cached, _ := c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", digest)
	if st != idempHit {
...
func TestIdempotency_ConflictDifferentPayload(t *testing.T) {
	...
	st, _, _ := c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", d1)
	if st != idempMiss {
		t.Fatalf("first lookup want miss, got %d", st)
	}
	c.complete(1, MsgTypeExtractorResolve, "r1", d1, []byte(`ok`))
	st, _, _ = c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", d2)
	if st != idempConflict {
...
func TestIdempotency_EvictOldestCompleted(t *testing.T) {
	...
	for i, id := range []string{"a", "b"} {
		...
		c.lookupOrBegin(1, MsgTypeExtractorResolve, id, d)
		c.complete(1, MsgTypeExtractorResolve, id, d, []byte(id))
	}
	d3 := canonicalDigest(json.RawMessage(`{"n":3}`))
	c.lookupOrBegin(1, MsgTypeExtractorResolve, "c", d3)
	...
	st, _, _ := c.lookupOrBegin(1, MsgTypeExtractorResolve, "a", d1)
	if st == idempHit {
...
func TestIdempotency_ExpiryIgnoresHit(t *testing.T) {
	...
	c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", d)
	c.complete(1, MsgTypeExtractorResolve, "r1", d, []byte("ack"))
	time.Sleep(40 * time.Millisecond)
	st, _, _ := c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", d)
	if st != idempMiss {
...
func TestIdempotency_InFlightCoalesce(t *testing.T) {
	...
	st, _, wait := c.lookupOrBegin(1, MsgTypeExtractorResolve, "r1", d)
	if st != idempCoalesce {
