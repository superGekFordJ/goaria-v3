...
func TestStartPairing_ResetsPairedState(t *testing.T) {
	...
	// Old secret replaced with a fresh one generated by the new pairing cycle.
	if store.GetSecret() == "old-secret" {
		t.Fatal("old secret should be replaced by a fresh one from the new pairing cycle")
	}

	// Complete an auth with the new secret and verify NotifyPaired fires + service stops.
	...
	secret := store.GetSecret()
	if secret == "" {
		t.Fatal("new secret should be generated by pairing Start")
	}
	if err := srv.Start(0); err != nil {
