...
func TestIsEndGame_NoActiveTasks(t *testing.T) {
	...
	queue := NewTaskQueue()
	// Simulate an idle worker by incrementing idleWorkers
...
func TestIsEndGame_NoIdleWorkers(t *testing.T) {
	...
	queue := NewTaskQueue()
	// No idle workers — idleWorkers stays 0
...
func TestIsEndGame_True(t *testing.T) {
	...
	queue := NewTaskQueue()
	queue.idleWorkers.Add(1) // simulate idle worker
...
func TestHedgeAll_SkipsHedged(t *testing.T) {
	d := makeEndGameDownloader([]struct {
		...
		{offset: 0, length: 100, hedged: 0},
		{offset: 100, length: 200, hedged: 1}, // already hedged
		{offset: 300, length: 300, hedged: 0},
	})
...
func TestHedgeAll_SkipsZeroRemaining(t *testing.T) {
	d := makeEndGameDownloader([]struct {
		...
		{offset: 100, length: 0, hedged: 0}, // current=100, stop=100 → 0 remaining
	})
...
func TestEndGame_HedgeAllIntegratesWithBalancer(t *testing.T) {
	...
	queue := NewTaskQueue()
	queue.idleWorkers.Add(2) // simulate 2 idle workers
	...
	// HedgeAll should create 2 hedge tasks
	n := d.HedgeAll(queue)
	if n != 2 {
		t.Fatalf("HedgeAll returned %d, want 2", n)
	}

	// Queue should now have 2 tasks for idle workers to pick up
	if queue.Len() != 2 {
		t.Errorf("queue should have 2 hedge tasks, got %d", queue.Len())
	}

	// Subsequent HedgeWork should return false (all already hedged)
	if d.HedgeWork(queue) {
...
func TestEndGame_ConcurrentAccess(t *testing.T) {
	...
	// Goroutine 1: repeatedly call recordHedgeError and recordHedgeSuccess
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < 200; i++ {
	...
	// Goroutine 2: repeatedly call HedgeAll
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < 200; i++ {
			// Reset hedged flags so HedgeAll can hedge again
			d.activeMu.Lock()
	...
	// Goroutine 3: repeatedly call HedgeWork
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < 200; i++ {
	...
	// Goroutine 4: repeatedly read hedgeDisabled and consecutiveHedgeErrors
	wg.Add(1)
	go func() {
		defer wg.Done()
		for i := 0; i < 200; i++ {
