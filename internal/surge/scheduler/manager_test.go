...
func TestRunDownload_SingleEndgameLatePauseFinalizesComplete(t *testing.T) {
	tmpDir := t.TempDir()
	body := []byte("single stream content for late pause endgame test")

	progState := progress.New("single-late-pause", int64(len(body)))
	...
	// Simulate ProgressState already having late-pause flag set by the race
	progState.Paused.Store(true)

	destPath := filepath.Join(tmpDir, "single_late_pause.bin")
	if f, err := os.Create(destPath + types.IncompleteSuffix); err == nil {
		_ = f.Close()
	} else {
		t.Fatal(err)
	...
	cfg := types.DownloadRecord{
		URL:                  server.URL,
		OutputPath:           tmpDir,
		Filename:             "single_late_pause.bin",
		ID:                   "single-late-pause",
		ProgressCh:           progressCh,
		ProgressState:        progState,
		Runtime:              &types.RuntimeConfig{},
	...
	err := RunDownload(ctx, &cfg)
	if err != nil {
		t.Fatalf("RunDownload should succeed with nil despite late pause, got: %v", err)
	}

	// Verify EventComplete is emitted
	...
	for msg := range progressCh {
		...
		if msg.Type == types.EventPaused {
			t.Fatalf("unexpected EventPaused when physical download succeeded: %+v", msg)
		}
		if msg.Type == types.EventComplete {
			completeReceived = true
			if msg.Total != int64(len(body)) {
				t.Errorf("EventComplete.Total = %d, want %d", msg.Total, len(body))
			}
		}
	}

	if !completeReceived {
		t.Fatal("expected EventComplete to be delivered despite late pause")
	}
	...
	if string(downloadedData) != string(body) {
		t.Errorf("downloaded content mismatch: got %q, want %q", string(downloadedData), string(body))
	}
}

func TestRunDownload_ConcurrentPauseReturnsErrPaused(t *testing.T) {
	tmpDir := t.TempDir()
	...
	progressCh := make(chan types.DownloadEvent, 100)
	progState := progress.New("test-pause-err", fileSize)
	cfg := types.DownloadRecord{
		URL:           server.URL(),
		OutputPath:    tmpDir,
		Filename:      "pause.bin",
	...
	case err := <-errCh:
		if !errors.Is(err, types.ErrPaused) {
			t.Fatalf("expected RunDownload to return types.ErrPaused, got %v", err)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for RunDownload after pause")
	}
}
