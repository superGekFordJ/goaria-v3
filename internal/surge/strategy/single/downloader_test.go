...
func TestSingleDownloader_PreallocateFailure_ReleasesFileHandle(t *testing.T) {
	...
	// Cenário: criar o .surge como read-only para que preallocate.Preallocate falhe
...
func swapHostRateLimiter(t *testing.T) *transport.HostRateLimiter {
	t.Helper()
	old := transport.DefaultHostRateLimiter
	isolated := transport.NewHostRateLimiter()
	transport.DefaultHostRateLimiter = isolated
	t.Cleanup(func() {
		transport.DefaultHostRateLimiter = old
	})
	return isolated
}
...
func TestSingleDownloader_RecordSuccessClearsPenalty(t *testing.T) {
	...
	destPath := filepath.Join(tmpDir, "rs.bin")
	state := progress.New("rs-test", fileSize)
	downloader := NewSingleDownloader("rs-id", nil, state, &types.RuntimeConfig{})
...
func TestSingleDownloader_PenalizeOn429(t *testing.T) {
	limiter := swapHostRateLimiter(t)

	tmpDir, cleanup, err := testutil.TempDir("surge-single-penalize")
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()

	fileSize := int64(2 * utils.KiB)
	body := make([]byte, fileSize)
	var calls atomic.Int32
	var sawPenalty atomic.Bool
	...
	host := transport.MirrorHost(server.URL)
	destPath := filepath.Join(tmpDir, "penalize.bin")
	state := progress.New("penalize-test", fileSize)
	downloader := NewSingleDownloader("penalize-id", nil, state, &types.RuntimeConfig{})
	...
	start := time.Now()
	if err := downloader.Download(ctx, server.URL, destPath, fileSize, "penalize.bin"); err != nil {
		t.Fatalf("Download failed: %v", err)
	}
	elapsed := time.Since(start)
	<-done
	...
	if calls.Load() < 2 {
		t.Fatalf("expected at least 2 requests (429 then 200), got %d", calls.Load())
	}
	if !sawPenalty.Load() {
		t.Fatal("expected Penalize to set BlockedUntil during bare-429 backoff")
	}
	// Old path slept a fixed 5s on bare 429; Penalize first hit is ~1s (±jitter).
	if elapsed >= 4*time.Second {
		t.Fatalf("backoff elapsed %v looks like hard-coded 5s path, want Penalize (~1s)", elapsed)
	}
}

func TestSingleDownloader_RateLimitedClearedOnCancelDuringBackoff(t *testing.T) {
	_ = swapHostRateLimiter(t)

	tmpDir, cleanup, err := testutil.TempDir("surge-single-rl-cancel")
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
	...
	destPath := filepath.Join(tmpDir, "rl_cancel.bin")
	state := progress.New("rl-cancel-test", 1024)
	downloader := NewSingleDownloader("rl-cancel-id", nil, state, &types.RuntimeConfig{})
	...
	go func() {
		errCh <- downloader.Download(ctx, server.URL, destPath, 1024, "rl_cancel.bin")
	}()

	deadline := time.Now().Add(5 * time.Second)
	for !state.RateLimited.Load() {
		if time.Now().After(deadline) {
			t.Fatal("timed out waiting for RateLimited=true during backoff")
		}
		time.Sleep(5 * time.Millisecond)
	}
	cancel()
	...
	case err := <-errCh:
		if err == nil {
			t.Fatal("expected cancel error from Download")
		}
		if !errors.Is(err, context.Canceled) {
	...
	if state.RateLimited.Load() {
		t.Fatal("RateLimited should be false after cancel during backoff")
	}
}
