...
// used by BitmapTracker so RecalculateProgress trusts the bitmap.
...
// resume helpers exercise the same on-disk shape as Download().
func saveResumeState(t *testing.T, id, url, destPath string, savedState *types.DownloadRecord, fileSize int64) {
	t.Helper()
	...
	if err := store.SaveState(url, destPath, savedState); err != nil {
		t.Fatal(err)
	}
...
func TestResume_BitmapTrust_InflatedDownloadedDoesNotOverrideVP(t *testing.T) {
	...
	progState := progress.New("inflate-multi", fileSize)
	...
	vp := progState.Bytes.VerifiedProgress.Load()
...
func TestResume_BitmapTrust_SingleChunkVPZeroInflatedDownloaded(t *testing.T) {
	...
	progState := progress.New("inflate-single", fileSize)
	progState.InitBitmap(fileSize, chunkSize)
...
func TestResume_NoBitmap_LegacyVPEqualsDownloaded(t *testing.T) {
	...
	progState := progress.New("legacy-nobitmap", fileSize)
	progState.InitBitmap(fileSize, chunkSize)
...
func TestResume_BitmapTrust_AllZeroBitmapBenignRegression(t *testing.T) {
	...
	progState := progress.New("allzero-bitmap", fileSize)
	progState.InitBitmap(fileSize, chunkSize)
...
func TestResume_BitmapTrust_DownloadedNotGreaterThanVP(t *testing.T) {
	...
	progState := progress.New("dl-le-vp", fileSize)
	progState.InitBitmap(fileSize, chunkSize)
