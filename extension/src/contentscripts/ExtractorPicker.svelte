<script lang="ts">
  ...
  let titleId = 'extractor-picker-title'
  ...
  $effect(() => {
    const next = pickerView.state.items
    const key = `${pickerView.state.pageToken}:${next.map(row => row.index).join(',')}`
          ...
          {#each visibleItems as item (item.index)}
            ...
            >
              ...
              <span class="extractor-picker-size">
                {typeof item.size_bytes === 'number' ? formatBytes(item.size_bytes) : t('picker_size_unknown')}
              </span>
            </label>
          {/each}
        </div>
